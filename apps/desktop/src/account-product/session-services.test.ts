...
const input = {
  runtime: {
    ...
    workspaceRoot: "/workspace", name: "Alice Mac", adapter: {} as never, privateConfigurationDirectory: "/private", encryption: {} as never,
  },
  mcp: {
...
describe("AccountProductSessionServices", () => {
  it("starts Runtime and installs the Account MCP with the private loopback config", async () => {
    const runtime = { start: vi.fn().mockResolvedValue({ runtimeId: "runtime:one" }), refresh: vi.fn(), stop: vi.fn().mockResolvedValue(undefined) };
    ...
    await expect(services.start(input)).resolves.toEqual({ runtime: { state: "ready", runtimeId: "runtime:one" }, mcp: { state: "ready" } });
  ...
  it("keeps MCP available when the local Runtime is unavailable and revokes both on stop", async () => {
    ...
    await expect(services.start(input)).resolves.toEqual({ runtime: { state: "failed", errorCode: "runtime-detection-failed" }, mcp: { state: "ready" } });
    await services.stop();
  ...
  it("bounds a hung local service without blocking Account entry", async () => {
    ...
    await expect(services.start(input)).resolves.toEqual({ runtime: { state: "failed", errorCode: "runtime-start-timeout" }, mcp: { state: "ready" } });
  });
  ...
  it("delegates Runtime refresh only to the active local service", async () => {
    ...
    const runtime = { start: vi.fn().mockResolvedValue({ runtimeId: "runtime:one" }), refresh: vi.fn().mockResolvedValue(refreshed), stop: vi.fn().mockResolvedValue(undefined) };
