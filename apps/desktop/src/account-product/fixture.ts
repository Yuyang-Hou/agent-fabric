import { agentDetailProjectionSchema, agentDraftSchema, runtimeSchema } from "@agent-fabric/account-agent-domain";
import { accountProductRendererCommandResultSchema, accountProductRendererSnapshotSchema, type AccountProductRendererCommand, type AccountProductRendererSnapshot } from "./ipc.js";
...
const agents = [
  ...
  { id: "agent:draft", name: "Knowledge Curator", description: "整理团队知识并生成来源清晰的摘要。", status: "needs_runtime", runtime: undefined, owner, access: "owner", runs: undefined, active: undefined, version: 1 },
] as const;
...
const base = accountProductRendererSnapshotSchema.parse({
  ...
  drafts: [], runtimes: [runtime, agents[1].runtime], runtimeDetail: undefined,
...
function fixtureCommand(current: AccountProductRendererSnapshot, command: AccountProductRendererCommand): AccountProductRendererSnapshot {
  switch (command.type) {
    ...
    case "draft-create": { const draft = draftFixture(command.mode, command.templateId); return { ...current, route: command.mode === "ai" ? { name: "agent-create-ai", draftId: draft.draftId } : { name: "agent-create-manual", draftId: draft.draftId }, activeDraft: draft, drafts: [draft] }; }
    case "logout": return { ...current, session: { state: "signed-out", reason: "logged_out" }, route: { name: "agents" } };
    default: return current;
  }
}

function draftFixture(mode: "blank" | "template" | "ai", templateId?: string) {
  return agentDraftSchema.parse({
    draftId: `draft:${mode}`, accountId, ownerUserId: owner.userId, mode, ...(templateId ? { templateId } : {}), name: mode === "template" ? "研究助手" : mode === "ai" ? "需求分析助手" : "", description: mode === "template" ? "把来源整理成简洁结论。" : mode === "ai" ? "澄清需求并生成风险清单。" : "", runtimeId: runtime.runtimeId, permissionMode: "private", configuration: { ...configuration, instructions: mode === "blank" ? "" : configuration.instructions }, pendingUserText: "", ...(mode === "ai" ? { builderSession: { state: "idle", conversation: [{ messageId: "message:builder", role: "assistant", text: "我已经准备好草稿。这个智能体主要服务哪些好友？", createdAt: at }] } } : {}), state: "active", createdAt: at, updatedAt: at, expiresAt: "2026-08-20T08:30:00.000Z", version: 2,
  });
}
...
function withRoute(snapshot: AccountProductRendererSnapshot, route: string): AccountProductRendererSnapshot {
  ...
  if (route === "manual") return fixtureCommand(snapshot, { type: "draft-create", mode: "blank" });
  if (route === "builder") return fixtureCommand(snapshot, { type: "draft-create", mode: "ai" });
