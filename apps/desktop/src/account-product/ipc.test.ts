...
describe("Account product Renderer IPC", () => {
  it("keeps Account collection snapshots free of private Agent and credential fields", () => {
    const snapshot = accountProductRendererSnapshotSchema.parse({
      ...
      runtimes: [], activities: [], templates: [], drafts: [], friends: [], incomingFriendInvitations: [], outgoingFriendInvitations: [], legacyRecovery: { state: "not_required" },
  ...
  it("keeps sensitive recovery output bounded to field categories and backup identity", () => {
    const base = {
      ...
      loading: false, refreshing: false, activities: [], templates: [], drafts: [], runtimes: [], friends: [], incomingFriendInvitations: [], outgoingFriendInvitations: [],
    };
