...
function RuntimeCatalog({ snapshot, busy, run }: ViewProps) {
  ...
  const visible = snapshot.runtimes
    .filter((runtime) => (!search.trim() || `${runtime.name} ${runtime.provider} ${runtime.adapterId}`.toLocaleLowerCase().includes(search.trim().toLocaleLowerCase())) && (health === "all" || runtime.health === health))
    .slice()
    .sort((a, b) => a.createdAt.localeCompare(b.createdAt) || a.runtimeId.localeCompare(b.runtimeId));
  const table = useTable({ features: collectionTableFeatures, columns: [], data: visible, getRowId: (runtime) => runtime.runtimeId });
  const hasLocalRuntimes = snapshot.localServices.runtimes.length > 0;
  return <PageSurface collection icon={Monitor} title="运行时" count={snapshot.runtimes.length} description="本机和 Account 中为智能体提供真实执行能力的 Runtime。" action={<FabricButton loading={busy === "local-runtimes-refresh-all"} disabled={!hasLocalRuntimes} onClick={() => void run({ type: "local-runtimes-refresh-all" })}><RefreshCw aria-hidden="true" />刷新本机检测</FabricButton>}>
...
function RuntimeDetailLoaded({ snapshot, busy, run, runtime }: ViewProps & { readonly runtime: NonNullable<AccountProductRendererSnapshot["runtimeDetail"]> }) {
  ...
  const isLocal = snapshot.localServices.runtimes.some((entry) => entry.runtimeId === runtime.runtimeId);
...
function LoginScreen({ snapshot, operation, run }: { readonly snapshot: AccountProductRendererSnapshot; readonly operation: AccountProductRendererCommand["type"] | undefined; readonly run: (command: AccountProductRendererCommand) => Promise<import("./ipc.js").AccountProductRendererCommandResult | false> }) {
  const googleBusy = operation === "login-google" || snapshot.session.state === "signing-in";
  return <main className="login-screen"><section className="login-panel"><span className="brand-symbol"><AgentFabricMark aria-hidden="true" /></span><h1>登录 Agent Fabric</h1><p role="status">邮箱登录暂未开放</p><FabricButton loading={googleBusy} onClick={() => void run({ type: "login-google" })}><LogIn aria-hidden="true" />{googleBusy ? "正在完成登录" : "使用 Google 登录"}</FabricButton><small><KeyRound aria-hidden="true" />第三方登录会话仅在认证过程中短暂使用。</small>{snapshot.errorCode && <div className="login-error" role="alert"><CircleAlert aria-hidden="true" /><span>{loginErrorMessage(snapshot.errorCode)}</span></div>}</section><div className="login-foot">Agent Fabric · 本地 Runtime 与私有凭据留在这台设备</div></main>;
}
